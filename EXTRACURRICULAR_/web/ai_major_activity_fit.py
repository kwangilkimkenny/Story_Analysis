...
totla_activity_num = len(total_actvity)
#print'(totla_activity_num :' totla_activity_num)
...
def input_majors(major_txt, act_txt_list):
    ...
    get_major = []
    n = 0
    if n <= len(act_title)-1: #모든 타이틀을 검사하기 위해서 총 개수와 같거나 작다면 아래 조건문 한번 실행
        for act_item in activity_list: # 입력한 활동관련 설명 10개의 리스트, 이것은 이미 사전에 토큰 처리됨(def activity_anaysis(text):)
            ...
            for j in act_item: #활동 관련 입력문장 개별 단어로 처리 잘됨
                ...
                for m in list(data_act['title']): #타이틀의 n번째 문장을 단어로 분해한 후 각 단어를 가져와서 활동관련 설명 단어와 비교한다.
                    #print('m :', m)
                    for k in m:                      
                        if j == k: # 매칭되는게 있다면, 해당 전공관련 주제를 추출해본다.
                            #print('FIT')                  
...
# 결과
# ===================================================
# RESULT : ['NOT FIT', 'FIT', 'FIT', 'FIT', 'NOT FIT', 'FIT', 'FIT', 'FIT']
# ===================================================