...
award_anaysis_re_TF = award_anaysis_re.isnull().values.any()#데이터 프레임에 값의 유무 확인. 부울 값을 반환한다. 값이 없으니까  Fasle이다.
...
if award_anaysis_re_TF == False:  ########## 여기 조건문을 고쳐야 함... 
    # 이것이 데이터베이스에 있는 수상명에 해당하는 점수 계산 결과다.
    # fit_re_fin
    fit_re_fin = sum(re_honor[["prestige_score"]].values.tolist(),[])
    print ("==== 수상내역 점수 ==================================")
    print ("수상점수 : ", fit_re_fin) 
else:
